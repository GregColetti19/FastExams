...
// files were sent — three 300MB lecture decks cost far more to ingest than
// three 2MB ones. A file-count cap would let one tester burn the whole alpha
// budget in three uploads, so the budget is denominated in bytes.
//
// Size is a proxy for spend, not a measure of it: a 300MB scanned PDF holds
// less text than a 20MB digital one. This is the cheap guard that keeps a
// runaway loop bounded; the provider-side spend cap is the real backstop.

export const DAILY_UPLOAD_BUDGET_MB = Number(process.env.DAILY_UPLOAD_BUDGET_MB || '400')
